...
for i in range(n):
   c = 0
   for j in my_range((file_length_in_bytes/n)*i+1, (file_length_in_bytes/n)*(i+1), 8):
...
for lin in range(len(lines)):
   if '"time":' in lines[lin]: 
      for i in range(n):
         if i == n-1:
            timings.append(float(lines[lin].split('[')[1].split(', ')[i].split(']')[0]))
         else:
            timings.append(float(lines[lin].split('[')[1].split(', ')[i]))

names = ['AR_NIC_ORB_PRF_RSP_BYTES_RCVD', 'AR_NIC_RSPMON_NPT_EVENT_CNTR_NL_FLITS', 'AR_NIC_RSPMON_NPT_EVENT_CNTR_NL_PKTS']

z = [(i - j) * 16 for i, j in zip(list(itertools.chain(*values[names[1]])), list(itertools.chain(*values[names[2]])))] #SEND bytes
