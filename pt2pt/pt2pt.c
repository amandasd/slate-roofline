...
main(int argc, char* argv[])
{
    ...
    // Since we only want to do a gather on every n'th rank, we need to create a new
    // MPI_Group
    ...
    InitAriesCounters(argv[0], myid, cpn, &AC_event_set, &AC_events, &AC_values,
                      &AC_event_count);
    // END ARIES
    ...
    StartRecordAriesCounters(myid, cpn, &AC_event_set, &AC_events, &AC_values,
                             &AC_event_count);
    ...
    for(k = 0; k < 1; k++)
    {
        if(myid == 0)
        {
            ...
            printf("Rank %d: received %d char(s) from rank %d with tag %d.\n", myid,
                   count, recv_status.MPI_SOURCE, recv_status.MPI_TAG);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    EndRecordAriesCounters(myid, cpn, &AC_event_set, &AC_events, &AC_values,
                           &AC_event_count);

    FinalizeAriesCounters(&mod_comm, myid, cpn, &AC_event_set, &AC_events, &AC_values,
                          &AC_event_count);
    MPI_Finalize();
