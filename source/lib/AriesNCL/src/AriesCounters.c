...
/* library methods */
void
ReadAriesCountersFile(char*** AC_events, int* AC_event_count)
{
    FILE* fp;
    int   myrank;
    ...
    // we are relying on \n, so it will cause errors later if the file pointer does not
    // end in a blank line.
    int linecount = 0;
    while(!feof(fp))
    {
...
void
InitAriesCounters(char* progname, int my_rank, int reporting_rank_mod, int* AC_event_set,
                  char*** AC_events, long long** AC_values, int* AC_event_count)
{
    ...
    *AC_values = (long long*) malloc(sizeof(long long) * (*AC_event_count));
    ...
    number_of_reporting_ranks = (size - 1) / reporting_rank_mod +
                                1;  // integer division is fine; we want the floor
    ...
    if(my_rank == 0)
    {
        // We want to get just the executable name, not the whole directory if we have an
        // absolute path
        char* exe_name;
...
void
StartRecordAriesCounters(int my_rank, int reporting_rank_mod, int* AC_event_set,
                         char*** AC_events, long long** AC_values, int* AC_event_count)
{
    if(my_rank % reporting_rank_mod != 0)
    {
 ...
 */
void
EndRecordAriesCounters(int my_rank, int reporting_rank_mod, int* AC_event_set,
                       char*** AC_events, long long** AC_values, int* AC_event_count)
{
    if(my_rank % reporting_rank_mod != 0)
    {
    ...
    struct timestep_counters* new_counters =
        (struct timestep_counters*) malloc(sizeof(struct timestep_counters));
    // Copy the counters.
...
void
FinalizeAriesCounters(MPI_Comm* mod16_comm, int my_rank, int reporting_rank_mod,
                      int* AC_event_set, char*** AC_events, long long** AC_values,
                      int* AC_event_count)
{
    if(my_rank % reporting_rank_mod != 0)
    {
    ...
    long long* counter_data = (long long*) malloc(
        sizeof(long long) * number_of_reporting_ranks * *AC_event_count);
    unsigned long long* timer_data = (unsigned long long*) malloc(
        sizeof(unsigned long long) * number_of_reporting_ranks);
    char json_filename[50];
    char bin_filename[50];

    // Walk the list of timestep counters, and for each one print out the data to a binary
    // file.
    ...
    while(ref)
    {
        ...
        MPI_Gather(ref->counters, *AC_event_count, MPI_LONG_LONG, counter_data,
                   *AC_event_count, MPI_LONG_LONG, 0, *mod16_comm);

        /* MPI_Gather to collect timing from all ranks to 0 */
        MPI_Gather(&(ref->elapsed_time), 1, MPI_LONG_LONG, timer_data, 1,
                   MPI_LONG_LONG_INT, 0, *mod16_comm);

        /* Write out counter_data in binary and timer_data in text, and read back in later
         * to generate yaml. */
        if(my_rank == 0)
        {
            ...
            WriteAriesCounters(number_of_reporting_ranks, reporting_rank_mod,
                               counter_data, timer_data, timestep, json_filename,
                               bin_filename, AC_events, AC_event_count);
        }
...
void
WriteAriesCounters(int number_of_reporting_ranks, int reporting_rank_mod,
                   long long* counter_data, unsigned long long* timer_data, int timestep,
                   char* jsonfile, char* binfile, char*** AC_events, int* AC_event_count)
{
    int   i, j;
    FILE* fp;
    ...
    fp = fopen(binfile, "wb");
    fwrite(counter_data, sizeof(long long), number_of_reporting_ranks * *AC_event_count,
           fp);
    fclose(fp);
#endif
}
